/**
 * BluePayPayment is an interface to Bluepay's payment gateway. Included are
 * functions to call numerous BluePay APIs for doing transactions, getting
 * report data, etc.
 */
...
public class BluePay {
     ...
     *
     * @param merchant A string containing the merchant's Account ID. A 12-digit
     * numeral.
     * @param secretKey A string containing the merchant's Secret Key. 32
     * characters, alphanumeric.
     * @param mode A string indicating the desired processing mode, "TEST" or
     * "LIVE"
     *
     */
    ...
    /**
     * Sets up object to process a SALE. A SALE both authorizes the card and
     * captures the funds in one step.
     *
     * In general, a SALE is the correct transaction type to use. Use AUTH only
     * if you have special needs.
     *
     * @param amount A string containing the amount of the transaction, e.g.
     * "10.00"
     * @param transactionID - the transaction id of a previous sale. Uses the
     * customer and payment information of previous transaction.
     *
     */
    ...
    /**
     * Sets up the object to process an AUTH. An auth authorizes payment and
     * garuntees the funds for later CAPTURE, but it does not transfer funds.
     * You must perform a CAPTURE or use Autocap.
     *
     * @param amount A string containing the amount of the transaction, e.g.
     * "10.00"
     * @param transactionID An optional string containing the 12-digit
     * transaction ID of the transaction to run a sale against.
     *
     */
    ...
    /**
     * Sets up the object to perform a REFUND. The actual transaction performed
     * will depend on the original transaction status, especially whether it's
     * batch has settled or not.
     *
     * @param transactionID A string containing the 12-digit transaction ID of
     * the transaction to refund.
     * @param amount An optional string containing the amount to refund. By
     * default, the entire original transaction is refunded.
     *
     */
     ...
     *
     * @param transID A string containing the 12-digit transaction ID of the
     * AUTH to CAPTURE.
     * @param amount An optional string containing the amount to capture. By
     * default, the entire original amount is captured.
     *
     */
     ...
     *
     * @param cardnum A string containing the Credit card number, all digits --
     * do not include spaces or dashes.
     * @param expire A string containing the card's expiration date in MMYY
     * format.
     * @param cvv2 A (sometimes) optional string containing the Card
     * Verification Value -- the 3 digit number printed on the back of most
     * cards. Whether it is in fact optional depends on your credit card
     * processing network's requirements.
     *
     */
     ...
     *
     * @param trackData A string containing both track 1 and track 2 date
     * directly from the credit card reader.
     */
     ...
     *
     * @param routingNum A string containing the routing number (9 digits). Make
     * sure to include any leading zeros.
     * @param accountNum A string containing the account number. Make sure to
     * include any leading zeros.
     * @param accountType Account type of ACH account. Checking ('C'), Savings
     * ('S').
     * @param docType Documentation type of transaction. May be 'PPD', 'CCD',
    ...
    /**
     * Sets the customer information and billing address. While this is
     * technically optional, it is highly recommended. Some payment processors
     ...
     * @param address2 A string containing the second line of a street or unit
     * number.
     * @param city A string containing the billing city
     * @param state A string containing the billing state, province, or regional
     * equivalent
     ...
     *
     * @param phonenum A string containing the phone number. It should contain
    ...
    /**
     * Sets the customer's email address. Required if you expect them to get an
     * email receipt from Bluepay.
     *
     ...
     * @param firstDate A string containing the first rebilling date; can
     * contain either a date in ISO format or a date expression such as "1
     * MONTH" to begin rebilling 1 month from now.
     * @param expr A string containing the Rebilling Expression; this indicates
     * how often to rebill. E.g. "1 MONTH" will rebill monthly; "365 DAY" or "1
     * YEAR" will rebill annually.
     * @param cycles A string containing the number of times to rebill;
     * optional. Will rebill forever if not set.
     *
     */
     ...
     * @template_id the id of a previous transaction to be used as the payment
     * information for the rebill
     ...
     * @param cycles A string containing the number of times to rebill;
     * optional. Will rebill forever if not set.
     * @param rebillAmount A string containing the amount to charge each time
     * the rebilling is run.
     * @param nextAmount A string containing the *next* amount to charge the
     * customer.
     *
     */
     ...
     * @param subaccountsSearched Either a 1 or a 0 to indicate whether to
     * search subaccounts as well as the main account
     * @param doNotEscape Either a 1 or a 0 to indicate whether the report
     * should take off quotes around the data
     * @param errors Either a 1 or a 0 to indicate whether the report should
     * exclude errored transactions
     *
     */
    ...
    /**
     * Gets report of settled transaction data based upon a start and end date
     * range.
     *
     * @param reportStart A string containing the date to start the report
     * @param reportEnd A string containing the date to stop the report
     ...
     * @param reportStart A string containing the date to start the report;
     * required for tamper proof seal
     * @param reportEnd A string containing the date to stop the report;
     * required for tamper proof seal
     * @param errors Either a 1 or a 0 to indicate whether the query should
     * exclude errored transactions
     *
     */
    ...
    /**
     * Queries transactions by a specific Payment Type. Must be used with
     * getSingleTransQuery
     *
    ...
    /**
     * Queries transactions by a specific Transaction Type. Must be used with
     * getSingleTransQuery
     *
    ...
    /**
     * Queries transactions by a specific Transaction Amount. Must be used with
     * getSingleTransQuery
     *
    ...
    /**
     * Queries transactions by a specific First Name. Must be used with
     * getSingleTransQuery
     *
    ...
    /**
     * Queries transactions by a specific Last Name. Must be used with
     * getSingleTransQuery
     *
    ...
    public HashMap<String, String> process() throws ClientProtocolException, IOException, NoSuchAlgorithmException {
        ...
        System.out.println("BPURL:"+BP_URL);
        ...
        if (BP_URL.equals("https://secure.bluepay.com/interfaces/bp10emu")) {
            ...
            System.out.println("IF RES:"+response);
            return response;
        } else {
            ...
            while ((line = rd.readLine()) != null) {
                List<NameValuePair> params;
                try {
                    //params = URLEncodedUtils.parse(line, Charset.defaultCharset());
                    params = URLEncodedUtils.parse(new URI(line), "UTF-8");
                    for (NameValuePair nameValuePair : params) {
                        response.put(nameValuePair.getName(), nameValuePair.getValue());
                    }
                } catch (URISyntaxException ex) {
                    Logger.getLogger(BluePay.class.getName()).log(Level.SEVERE, null, ex);
                }

            }
            System.out.println("ELSE RES:"+response);
            return response;
        }
    }
    ...
    /**
     * Returns true if the transaction was successful (i.e. was approved and not
     * a duplicate)
     *
     * @return true if error; false otherwise.
     *
     ...
     *
     * @return A string containing the status; e.g. "Approved" or "Declined:
     * Hold Card"
     *
     */
     ...
     *
     * @return String containing the Rebill Creation Date response or null if
     * none.
     *
     */
     ...
     *
     * @return String containing the Rebill Schedule Expression response or null
     * if none.
     *
     */
     ...
     *
     * @return String containing the Rebill Cycles Remaining response or null if
     * none.
     *
     */
     ...
     *
     * @return String containing the Rebill Next Amount response or null if
     * none.
     *
     */
